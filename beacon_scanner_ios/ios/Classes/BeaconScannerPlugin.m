...
@interface BeaconScannerPlugin() <CLLocationManagerDelegate, CBCentralManagerDelegate, CBPeripheralManagerDelegate>
...
@property (strong, nonatomic) BSRangingStreamHandler* rangingHandler;
@property (strong, nonatomic) BSMonitoringStreamHandler* monitoringHandler;
@property (strong, nonatomic) BSBluetoothStateHandler* bluetoothHandler;
@property (strong, nonatomic) BSAuthorizationStatusHandler* authorizationHandler;
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel methodChannelWithName:@"plugins.lukangagames.com/beacon_scanner_android"
                                                                binaryMessenger:[registrar messenger]];
    BeaconScannerPlugin* instance = [[BeaconScannerPlugin alloc] init];
...
-(void)locationManager:(CLLocationManager*)manager didRangeBeacons:(NSArray*)beacons inRegion:(CLBeaconRegion*)region{
    if (self.flutterEventSinkRanging) {
        NSDictionary *dictRegion = [BSUtils dictionaryFromCLBeaconRegion:region];
        
        NSMutableArray *array = [NSMutableArray array];
        for (CLBeacon *beacon in beacons) {
            NSDictionary *dictBeacon = [BSUtils dictionaryFromCLBeacon:beacon];
...
- (void)locationManager:(CLLocationManager *)manager didEnterRegion:(CLRegion *)region {
    if (self.flutterEventSinkMonitoring) {
        ...
        if (reg) {
            NSDictionary *dictRegion = [BSUtils dictionaryFromCLBeaconRegion:reg];
